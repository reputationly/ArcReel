MESSAGES = {
    ...
    "shots_mode_required": "this script is not a shot-list mode (ad or MV); use the endpoint for its own mode",
    ...
    "prompt_not_accepted": "{task_type} task does not accept a prompt (the target song and voice reference determine the output)",
    ...
    "mode_no_default_duration": "this content mode has no per-shot duration preference: ad plans shots against the target total, MV follows the song sections",
    "generation_mode_not_supported": "this content mode does not support the selected generation mode: {mode}",
    ...
    "video_reference_mode_unsupported": "Model {model} does not support reference-to-video (it only accepts a start frame), so every reference image was dropped and nothing is left to generate from; switch the project's storyboard mode to image-to-video, or pick a video model that supports multi-image references",
