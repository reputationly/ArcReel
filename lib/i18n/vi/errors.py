MESSAGES = {
    ...
    "shots_mode_required": "kịch bản này không thuộc chế độ danh sách cảnh quay (quảng cáo hoặc MV); hãy dùng endpoint của chế độ tương ứng",
    ...
    "prompt_not_accepted": "tác vụ {task_type} không nhận prompt (bài hát đích và giọng tham chiếu quyết định kết quả)",
    ...
    "mode_no_default_duration": "chế độ nội dung này không có tùy chọn thời lượng từng cảnh: quảng cáo lập theo tổng thời lượng mục tiêu, MV theo đoạn nhạc",
    "generation_mode_not_supported": "chế độ nội dung này không hỗ trợ phương thức tạo đã chọn: {mode}",
    ...
    "video_reference_mode_unsupported": "Mô hình {model} không hỗ trợ tạo video trực tiếp từ ảnh tham chiếu (chỉ nhận khung hình đầu), nên toàn bộ ảnh tham chiếu đã bị loại bỏ và không còn gì để tạo video; hãy đổi chế độ tạo phân cảnh của dự án sang «tạo video từ ảnh», hoặc chọn mô hình video hỗ trợ nhiều ảnh tham chiếu",
