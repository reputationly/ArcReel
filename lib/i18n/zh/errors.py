MESSAGES = {
    ...
    "shots_mode_required": "该剧本不是镜头表模式（广告/短片或 MV），请使用对应模式的更新接口",
    ...
    "prompt_not_accepted": "{task_type} 任务不接受 prompt（唱什么由目标曲决定、用谁的嗓子由音色参考决定）",
    ...
    "mode_no_default_duration": "该内容模式不持有单镜时长偏好：广告/短片按目标总时长逐镜规划，MV 由歌曲段落决定",
    "generation_mode_not_supported": "该内容模式不支持所选生成方式: {mode}",
